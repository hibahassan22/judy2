...
export default function Layout({ children }) {
  ...
  return (
    <div className="flex h-screen bg-[#f5f0e8] font-sans" dir="rtl">

      {/* ===== Sidebar ===== */}
      <aside className="w-56 bg-[#1a1a1a] flex flex-col justify-between py-6 px-4 shrink-0">
        {/* Logo */}
        <div>
          <div className="flex justify-center mb-8">
            <div className="w-16 h-16 rounded-full flex items-center justify-center overflow-hidden">
              <img src="../judy.png" className="w-full h-full object-cover" />
            </div>
          </div>
          ...
          <nav className="space-y-1">
            {navItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-all text-right font-medium
                  ${location.pathname === item.route || (item.route !== "/" && location.pathname.startsWith(item.route))
                    ? "bg-gradient-to-l from-[#9C6402] to-[#E6C76A] text-white shadow-md shadow-[#9C6402]/50"
        ...
        {/* Logout */}
        <button className="flex items-center gap-3 px-3 py-2.5 text-gray-400 hover:text-white text-sm transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
