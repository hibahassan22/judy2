...
const AddClientModal = ({ isOpen, onClose, onAddClient }) => {
  const [formData, setFormData] = useState({ name: "", phone: "", address: "", gender: "أنثى" });
  ...
  const handleSubmit = (e) => {
    ...
    setFormData({ name: "", phone: "", address: "", gender: "أنثى" });
    onClose();
  };
...
const ClientDetailsModal = ({ isOpen, onClose, client, onUpdateClient }) => {
  const [activeTab, setActiveTab] = useState("basic"); // basic | notes | trips
  const [editedClient, setEditedClient] = useState(null);

  // تحديث البيانات المحلية للمودال عند تغير العميل المحدد
  if (isOpen && (!editedClient || editedClient.id !== client.id)) {
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto">
          ...
          {activeTab === "notes" && (
            <div className="space-y-4">
              ...
              <button className="w-full mt-2 bg-[#4A4A4A] text-white font-medium py-3 rounded-xl hover:bg-[#3d3d3d] transition-colors text-center shadow-sm">
                إضافة ملاحظة
              </button>
            </div>
          )}
            ...
            <div className="space-y-3">
              {editedClient.tripHistory && editedClient.tripHistory.length > 0 ? (
                <>
                  ...
                  <button className="w-full border border-gray-200 text-gray-500 text-sm py-2.5 rounded-xl hover:bg-gray-50 transition-colors text-center mt-4 flex items-center justify-center gap-1">
    ...
    <div className="w-full space-y-4 p-4 md:p-6" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-1.5 bg-white border border-gray-200 text-gray-600 text-xs px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm"
        >
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-gray-400">تابع كل بيانات العملاء بسهولة</p>
        </div>
      </div>
